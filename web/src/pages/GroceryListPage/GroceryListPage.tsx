...
import type {
  ...
  // Types for the new batch delete mutation (will be generated)
  DeleteGroceryListItemsByCategoryMutation,
  DeleteGroceryListItemsByCategoryMutationVariables,
} from 'types/graphql'
...
const DELETE_GROCERY_LIST_ITEMS_BY_CATEGORY_MUTATION = gql`
  mutation DeleteGroceryListItemsByCategoryMutation($category: String!) {
    deleteGroceryListItemsByCategory(category: $category) {
      count
    }
  }
`
...
const GroceryListPage: React.FC = () => {
  ...
  const [
    ...
    { loading: _upsertPantryLoading, error: _upsertPantryError },
  ] = useMutation<
    UpsertPantryItemFromGroceryItemMutation,
    UpsertPantryItemFromGroceryItemMutationVariables
  ...
  const [
    deleteGroceryListItemsByCategory,
    { loading: deleteByCategoryLoading, error: deleteByCategoryError },
  ] = useMutation<
    DeleteGroceryListItemsByCategoryMutation, // Placeholder type
    DeleteGroceryListItemsByCategoryMutationVariables // Placeholder type
  >(DELETE_GROCERY_LIST_ITEMS_BY_CATEGORY_MUTATION, {
    onCompleted: (data) => {
      toast.success(
        `${data.deleteGroceryListItemsByCategory.count} item(s) deleted.`
      )
      refetch() // Refetch the grocery list
    },
    onError: (error) => {
      toast.error(`Failed to delete items by category: ${error.message}`)
    },
    // No need to refetchQueries here if onCompleted handles refetch()
    // but can be added for safety: refetchQueries: [{ query: GET_GROCERY_LIST_ITEMS_QUERY }]
  })
  ...
  const handleDeleteCategoryItems = async (categoryToDelete: Category) => {
    const itemsInCategory = groupedItems[categoryToDelete] || []
    if (itemsInCategory.length === 0) {
      toast(`No items to delete in ${categoryToDelete}.`) // Changed from toast.info
      return
    }

    const confirmDelete = window.confirm(
      `Are you sure you want to delete all ${itemsInCategory.length} items in ${categoryToDelete}?`
    )
    if (!confirmDelete) {
      return
    }
    ...
    try {
      await deleteGroceryListItemsByCategory({
        variables: { category: categoryToDelete },
      })
      // Toast notifications are handled by the useMutation's onCompleted/onError
      // refetch() is also handled by onCompleted
    } catch (error) {
      // This catch block might be redundant if useMutation's onError handles all errors.
      // However, it can catch errors if the mutation call itself fails synchronously before the promise resolves/rejects.
