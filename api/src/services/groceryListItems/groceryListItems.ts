...
export const createMultipleGroceryListItems: MutationResolvers['createMultipleGroceryListItems'] =
  async ({ inputs }) => {
    ...
    console.log('[CMGLI] Fetched existingDbItems:', JSON.stringify(existingDbItems, null, 2));
    ...
    console.log('[CMGLI] Generated existingDbItemKeys:', Array.from(existingDbItemKeys));
    ...
    for (const input of inputs) {
      ...
      console.log(`[CMGLI] Processing input item: ${input.name}, category: ${input.category}. Generated key: ${currentInputKey}`);
      if (existingDbItemKeys.has(currentInputKey)) {
        skippedItemNames.push(input.name) // Add original name to list of skipped items
        console.log(`[CMGLI] Item ${input.name} (${currentInputKey}) marked as SKIPPED (already exists in DB).`);
      } else {
        // Not a DB duplicate, so add it for creation
        itemsToCreateData.push(input)
        console.log(`[CMGLI] Item ${input.name} (${currentInputKey}) marked for CREATION.`);
      }
    }

    console.log('[CMGLI] Items marked for creation (itemsToCreateData):', JSON.stringify(itemsToCreateData, null, 2));
    ...
    console.log('[CMGLI] Final addedItems:', JSON.stringify(addedItems, null, 2));
...
export const deleteGroceryListItemsByCategory: MutationResolvers['deleteGroceryListItemsByCategory'] =
  async ({ category }) => {
    ...
    try {
      ...
      console.log(`[DGBC] Successfully deleted ${result.count} items for category: ${category}`);
      return {
        count: result.count,
      }
    } catch (error) {
      console.error(`[DGBC] Error deleting grocery items by category ${category}:`, error)
