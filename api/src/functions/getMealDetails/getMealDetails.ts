...
async function getMealDetailsFromLLM(
  mealName: string
  ...
  if (!process.env.OPENAI_API_KEY) {
    logger.error(
      '[OpenAI] API key is not configured. Please set OPENAI_API_KEY environment variable in your .env file.'
    )
    ...
    return {
      ...
      ingredients: [
        'OpenAI API Key not set. Please configure it in your .env file.',
      ],
      nutrients: {
        calories: 'N/A',
        protein: 'N/A',
...
Ensure the entire output is only the JSON object, with no surrounding text or explanations.`
  ...
  try {
    ...
    try {
      ...
      logger.error(
        '[OpenAI] Failed to parse JSON response from API for meal:',
        mealName,
        { error: parseError.message, rawContent: content }
      )
      return null
    }
    ...
    if (parsedResponse.error) {
      logger.warn(
        `[OpenAI] Meal not found or invalid for ${mealName} according to LLM: ${parsedResponse.error}`
      )
      return null
    }

    if (
      parsedResponse.mealName &&
      ...
      logger.error(
        '[OpenAI] Parsed response does not match expected MealDetails structure for meal:',
        mealName,
        { parsedResponse }
      )
      return null
    ...
    logger.error(
      '[OpenAI] Error fetching details from OpenAI for meal:',
      mealName,
      { errorMessage: error.message, errorStatus: error.status }
    )
    if (error.status === 401) {
      logger.error(
        '[OpenAI] Authentication error. Please check your OPENAI_API_KEY.'
      )
...
export const handler = async (event: APIGatewayEvent, _context: Context) => {
  logger.info(
    `${event.httpMethod} ${event.path}: getMealDetails function invoked`
  )
  ...
  try {
    ...
    return {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        error: 'Failed to fetch meal details from LLM service.',
      }),
    }
  }
  ...
  if (!mealDetails) {
    logger.warn(
      `No details found by LLM for meal: ${mealNameQuery}. Returning a default response.`
    )
